...
const AdvancedPhase: React.FC<AdvancedPhaseProps> = ({ data, setData, onNext, onBack }) => {
  ...
  const OptionButton = ({ label, isActive, onClick, className = "" }: { label: string, isActive: boolean, onClick: () => void, className?: string }) => (
    <button
      onClick={onClick}
      className={`h-20 rounded-2xl border font-tech text-[10px] tracking-widest uppercase transition-all flex flex-col items-center justify-center text-center p-2 ${
        isActive
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2 h-20 bg-[#111] rounded-3xl p-2 border border-white/5">
        <button 
          ...
          disabled={disableYes}
          className={`rounded-2xl text-[10px] font-tech tracking-widest transition-all ${
            value === 1 
